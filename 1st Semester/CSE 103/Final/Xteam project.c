...
int i=0;
int count=0; //book structure
struct books
{
    char book_title[50];
    char book_author[30]; //structure
    float price;
    int copies;
...
void showBooks()
{
    ...
    if (count==0)
        ...
        {
            printf ("Book title: %s\n", b[i].book_title);
            printf ("Book author: %s\n", b[i].book_author); //showing book function
...
void addBook()
{
    ...
    printf ("Enter book price: "); //adding book function
    scanf  ("%f",&b[i].price);
    printf ("Enter number of copies: ");
    scanf  ("%d",&b[i].copies);
    i=i+1;
    count++; //This count will be use in count function
...
void searchBook()
{
    ...
    for(i=0;i<500;i++)
        {
            if(strcmp(s1,b[i].book_title) == 0) // here we used string.h Library
            {
                found=1;
                printf("Book Name   : %s\n", b[i].book_title);
...
int main()
{
    ...
    while(choice!=6)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            addBook();              //in main function we are just calling other function.
            break;
        case 4:
            searchBook();
            break;
